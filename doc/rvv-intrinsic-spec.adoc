...
For Zvdota family dot product instructions (widening and quad-widening), the intrinsics encode both the input vector type and the output vector type in the suffix to disambiguate between different signedness combinations and data types. The naming pattern is `{OP}_vv_{VS1_TYPE}{SEW}m{LMUL}_{OUTPUT_TYPE}{OSEW}m{OLMUL}`.
...
vuint32m1_t __riscv_vqwdotau_vv_u8mf8_u32m1(vuint32m1_t vd, vuint8mf8_t vs2, vunt8mf8_t vs1, size_t vl);
----
