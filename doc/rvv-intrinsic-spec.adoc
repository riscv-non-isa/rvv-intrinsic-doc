...
Only encoding the return type will cause naming collisions for the add-with-carry / subtract-with-borrow instruction intrinsics. The intrinsics encode the input vector type and the output mask vector type in the suffix.
