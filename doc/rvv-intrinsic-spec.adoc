...
[,c]
----
__riscv_{V_INSTRUCTION_MNEMONIC}_{OPERAND_MNEMONIC}_{RETURN_TYPE}_{ROUND_MODE}_{ALTFMT}_{POLICY}{(...)
----
...
* `ALTFMT` is the `_alt` suffix which is used by intrinsics that need vtype.altfmt to be set to 1(especially bfloat16 intrinsics).
