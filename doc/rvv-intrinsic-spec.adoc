...
NOTE: When policy is set to "agnostic", there is no guarantee of what will be in the tail/masked-off elements. Under this policy users should not assume the values within to be deterministic. For intrinsics with no `vd` operand, the compiler is free to pick any register as the destination register.
