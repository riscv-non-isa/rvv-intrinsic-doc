...
The implicit (overloaded) intrinsics, on the contrary, hide the explicit specifications for `vtype` control. The implicit (overloaded) intrinsics aim to provide a generic interface to let users put values of different EEW ^17^ and EMUL ^17^ as the input argument.
...
The implicit (overloaded) interface aims to provide a generic interface that takes values of different EEW and EMUL as the input. Therefore, the implicit intrinsics hide the EEW and EMUL encoded in the function name. The `_rm` prefix for explicit-`frm` intrinsics (<<control-of-frm>>) is also hidden. The intrinsics under this scheme are the "overloaded intrinsics", which in parallel we have the "non-overloaded intrinsics" defined under <<explicit-naming-scheme>>.
...
Since the main intent is to let the users put different value(s) of EEW and EMUL as input argument(s), the overloaded intrinsics do not hide the policy suffix. That is, the suffix listed under <<control-of-policy>> is not hidden and is still encoded in the function name.
...
Due to the limitations of the C language (without the aid of features like C++ templates), some intrinsics do not have an overloaded version. Therefore these intrinsics do not possess a simplified, EEW/EMUL-hidden interface. Please see <<unsupported-implicit-naming>> for more detail.
...
Widening intruction intrinsics (e.g. `vwadd`) have the same return type but different types of arguments. The operand mnemonics are encoded into their overloaded versions to help distinguish them.
...
Type with bold font is only available when `ELEN >= 64` (that is, unavailable under `Zve32*` and require at least `Zve64x`).
...
[[pseudo-vundefined]]
=== `vundefined`
The `vundefined` intrinsics are placeholders for `vset` and `vcreate` to represent unspecified values.
...
The intrinsics does not map to any real instruction. Whether if the implementation will generate vector move instructions is an optimization issue.
...
The `vlmul_trunc` intrinsics are syntactic sugars that essentially have the same semantic as `vget` with `idx=0`.

[[pseudo-vlmul_ext]]
=== `vlmul_ext`

The `vlmul_ext` intrinsics are syntactic sugars that essentially have the same semantic as `vset` with `idx=0`.

[[pseudo-vcreate]]
=== `vcreate`

The `vcreate` intrinsics are syntactic sugars for tuple types creation. They essentially have the same semantic as multiple `vset`-s filling in values accordingly into the tuple type.

== Programming Notes

=== The `vd` argument in the intrinsics

Intrinsics whose computation is relevant to the -value held in `vd` have a passthrough argument in them. The following list enumerates the intrinsics that has a passthrough argument. Please see the appendix for the exact prototypes of these intrinsics.
...
The intrinsics provide variants with operand mnemonics of `vv` and `vx`, but not `vi`. This was an intentional design to reduce the total amount of out-going intrinsics. The implementation is responsible to emit instructions with operand mnemonics of `vi` when an immediate that can be expressed within 5-bit is provided to the intrinsics.
