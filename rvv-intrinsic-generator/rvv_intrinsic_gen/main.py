...
def main():
  ...
  if mode in [
    ...
    g = generator.APITestGenerator(args.out, False, ToolChainType(args.toolchain_type),
                                   args.has_policy)

  elif mode == GenTypes.OVERLOADED_TEST:
    g = generator.APITestGenerator(args.out, True, ToolChainType(args.toolchain_type),
                                   args.has_policy)
  else:
    assert False
  if not args.skip_default_inst:
