...
- [RISC-V "V" Vector Extension Intrinsics](#risc-v-v-vector-extension-intrinsics)
  - [Introduction](#introduction)
  - [Availability](#availability)
  - [Type System](#type-system)
    - [Data Types](#data-types)
    - [Mask Types](#mask-types)
    - [Tuple Types for segment load/store](#tuple-types-for-segment-loadstore)
  - [Configuration-Setting and `vl` Argument](#configuration-setting-and-vl-argument)
  - [Naming Rules](#naming-rules)
  - [Exceptions in Naming](#exceptions-in-naming)
    - [Vector Stores](#vector-stores)
    - [Vector Integer Add-with-Carry / Subtract-with-Borrow Instructions](#vector-integer-add-with-carry--subtract-with-borrow-instructions)
    - [Comparison Instructions](#comparison-instructions)
    - [Reduction Instructions](#reduction-instructions)
    - [`vcpop.m` and `vfirst.m`](#vcpopm-and-vfirstm)
  ...
  - [Masked Intrinsics Without MaskedOff](#masked-intrinsics-without-maskedoff)
    - [Vector Stores](#vector-stores-1)
    - [Reduction Instructions](#reduction-instructions-1)
    - [Merge Instructions](#merge-instructions)
  - [Policy Intrinsic Functions](#policy-intrinsic-functions)
    - [Intrinsics without `dest` or `vd` argument](#intrinsics-without-dest-or-vd-argument)
    - [Intrinsics with the `dest` argument and doesn't have a masked type](#intrinsics-with-the-dest-argument-and-doesnt-have-a-masked-type)
    - [Intrinsics with the `vd` argument](#intrinsics-with-the-vd-argument)
    - [Intrinsics for the reduction instruction which only have tail policy.](#intrinsics-for-the-reduction-instruction-which-only-have-tail-policy)
    - [Intrinsics for the instructions which only have mask policy. (ex. vector comparison, `vmsbf.m`, `vmsif.m` and `vmsof.m`.)](#intrinsics-for-the-instructions-which-only-have-mask-policy-ex-vector-comparison-vmsbfm-vmsifm-and-vmsofm)
    - [Special cases](#special-cases)
    - [Explicit policy for overloaded intrinsic functions](#explicit-policy-for-overloaded-intrinsic-functions)
  - [Rounding mode in fixed-point intrinsics](#rounding-mode-in-fixed-point-intrinsics)
  - [Rounding mode in floating-point intrinsics](#rounding-mode-in-floating-point-intrinsics)
  - [Keep the Original Values of the Destination Vector](#keep-the-original-values-of-the-destination-vector)
  - [SEW and LMUL of Intrinsics](#sew-and-lmul-of-intrinsics)
  - [C Operators on RISC-V Vector Types](#c-operators-on-risc-v-vector-types)
  - [Utility Functions](#utility-functions)
    - [Vector Initialization](#vector-initialization)
    - [Reinterpret between floating point and integer types](#reinterpret-between-floating-point-and-integer-types)
    - [Reinterpret between signed and unsigned types](#reinterpret-between-signed-and-unsigned-types)
    - [Reinterpret between different SEW under the same LMUL](#reinterpret-between-different-sew-under-the-same-lmul)
    - [Reinterpret between vector boolean types and LMUL=1 (m1) vector integer types](#reinterpret-between-vector-boolean-types-and-lmul1-m1-vector-integer-types)
    - [LMUL truncation and LMUL extension functions](#lmul-truncation-and-lmul-extension-functions)
    - [Vector Insertion and Extraction functions](#vector-insertion-and-extraction-functions)
  - [Overloaded Interface](#overloaded-interface)
    - [The unsupported overloading functions](#the-unsupported-overloading-functions)
      - [Non-masked `vle`, `vleff` and `vlseg`](#non-masked-vle-vleff-and-vlseg)
      - [Non-masked `viota.m`](#non-masked-viotam)
      - [`vmv.v.x` and `vfmv.v.f`](#vmvvx-and-vfmvvf)
      - [`vmclr.m`, `vmset.m`, `vid.v`, `vundefined`](#vmclrm-vmsetm-vidv-vundefined)
    - [Special naming rule for some overloaded interfaces.](#special-naming-rule-for-some-overloaded-interfaces)
      - [Convert Instructions.](#convert-instructions)
      - [Reinterpret, LMUL truncation, LMUL extension and vector extraction functions.](#reinterpret-lmul-truncation-lmul-extension-and-vector-extraction-functions)
      - [Widening Vector-Scalar Arithmetic Instructions for vx/wx and vf/wf cases.](#widening-vector-scalar-arithmetic-instructions-for-vxwx-and-vfwf-cases)
      - [Vector Move Instructions.](#vector-move-instructions)
  - [Switching Vtype and Keep same VL after vsetvl instruction](#switching-vtype-and-keep-same-vl-after-vsetvl-instruction)
  - [Programming Note](#programming-note)
    - [Strided load/store with stride of 0 ](#strided-loadstore-with-stride-of-0-)
