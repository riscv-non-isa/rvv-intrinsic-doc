...
vbool1_t test_vmsif_m_b1_m(vbool1_t vm, vbool1_t vs2, size_t vl) {
  return __riscv_vmsif(vm, vs2, vl);
}
...
/* { dg-final { scan-assembler-times {vseti?vli\s+[a-z0-9]+,\s*[a-z0-9]+,\s*e[0-9]+,\s*mf?[1248],\s*t[au],\s*m[au]\s+vmsif\.[ivxfswum.]+\s+} 14 } } */
